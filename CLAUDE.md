...
| `bin/lib/dsh.js` | DeepSeek Harness (DSH) install/uninstall helper. Copies the hook-free skill subset (`caveman`, `caveman-commit`, `caveman-review`, `caveman-help`, `caveman-compress`) into `$DSH_HOME/skills/` and upserts/strips the marker-fenced ruleset block in `$DSH_HOME/AGENTS.md` (DSH's user-global instruction file, auto-loaded every session). Ownership-journaled via `bin/lib/owned-install.js`, idempotent. The embedded standalone fallback in `loadRuleBody(null)` must stay byte-equivalent to `src/rules/caveman-activate.md`. |
...
| DeepSeek Harness (DSH) | Unified installer (`dsh` provider in `bin/install.js`, helper `bin/lib/dsh.js`): copies the hook-free skill subset (`caveman`, `caveman-commit`, `caveman-review`, `caveman-help`, `caveman-compress`) into `$DSH_HOME/skills/` (default `~/.dsh/skills/`, `DSH_HOME` honored) and upserts the marker-fenced always-on ruleset block into `$DSH_HOME/AGENTS.md` (DSH loads it as user-global instructions in every session). Ownership-journaled; `--uninstall` prunes both. Detection: `command:dsh \|\| dir:$HOME/.dsh`. No hooks/statusline/stats in v1 — DSH has no Claude-hook equivalent; `/caveman` in the GUI works through DSH's user-invocable skill gesture. **`npx skills add ... -a dsh` does NOT exist yet** — needs a `dsh` profile upstream in vercel-labs/skills (`src/agents.ts` + `src/types.ts`, then `scripts/sync-agents.ts`). | Yes — user-global AGENTS.md loads every session |
